 ...
 *                                                                          *
 *  vlynq_master( )                                                         *
 *      VLYNQ test, Master part that controls the clock and signals.        *
 *                                                                          *
 * ------------------------------------------------------------------------ */
Int16 vlynq_master( )
{
    ...
    while ( ( VLYNQ_STAT & 0x00000078 ) == 0 );

    /* Wait for the rx FIFO to empty */
    timeout = vlynq_timeout;
    while ( timeout-- > 0 )
        if ( ( VLYNQ_STAT & 0x00000078 ) == 0 )
            break;
    ...
    /* Wait so that the other board's system is stable */
    _waitusec( 10000 );
