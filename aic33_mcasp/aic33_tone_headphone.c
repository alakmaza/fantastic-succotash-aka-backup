 ...
 *                                                                          *
 * ------------------------------------------------------------------------ */
double LinearInterpolate(double y1, double y2, double mu, double roundmu);
double LinearInterpolate(double y1, double y2, double mu, double roundmu)
{
   return (y1*(1.0-(mu-roundmu))+y2*(mu-roundmu));
}
...
//    };
	double sinetable[48] = {
		 0.00, 4276.95, 8480.72, 12539.39, 16383.50, 19947.29, 23169.77, 25995.81, 28377.05, 30272.76,
		31650.49, 32486.67, 32767.00, 32486.67, 31650.49, 30272.76, 28377.05, 25995.81, 23169.77, 19947.29,
		16383.50, 12539.39, 8480.72, 4276.95,  0.00, -4276.95, -8480.72, -12539.39, -16383.50, -19947.29,
		-23169.77, -25995.81, -28377.05, -30272.76, -31650.49, -32486.67, -32767.00, -32486.67, -31650.49, -30272.76,
		-28377.05, -25995.81, -23169.77, -19947.29, -16383.50, -12539.39, -8480.72, -4276.95
	};
    ...
    /* Grab AIC33 handle */
    aic33handle = EVMDM6437_AIC33_openCodec( AIC33_MCASP_ID, &aic33config );

    /* Play Tone */
    double index = 0.0;
    double increment = frequency*48.0/fs;
    ...
    {
    	index += increment;
    	index = fmod( index, 48.0 );

    	double ind = floor(index);
    	thisOutput = (Int16) LinearInterpolate(sinetable[(Int16)ind], sinetable[(Int16)ceil(index)], index, ind);
//    	thisOutput = (Int16) LinearInterpolate(0.0, 32700., 0.5);
//    	thisOutput = (Int16) LinearInterpolate(16000., 32700., 0.0);
//    	// DEBUG
//
    	//myCount+=1;
    	//myCount %= 30000;
...
//        }
		...
		while ( ! EVMDM6437_AIC33_write16( aic33handle, thisOutput) );
